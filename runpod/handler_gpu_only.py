...
def load_gguf_model(model_path: str):
    ...
    try:
        ...
        # 计算适合的GPU层数
        # 对于大多数模型，可以将70-80%的层放在GPU上
        # L4 GPU有23GB显存，应该足够放置大部分层
        n_gpu_layers = 50  # 大量GPU层
        
        logger.info(f"使用强制GPU模式，{n_gpu_layers}层在GPU上")
        model = Llama(
            model_path=model_path,
            n_ctx=4096,           # 较大的上下文窗口
            n_batch=512,          # 较大的批处理大小
            n_gpu_layers=n_gpu_layers,  # 大量GPU层
            verbose=True,         # 详细日志
            use_mmap=True,
            use_mlock=False,
        )
        ...
        raise  # 重新抛出异常，因为我们不想回退到CPU模式

def initialize_model():
    """初始化模型，只使用GPU模式"""
    global model, model_type, model_path
    
    logger.info("开始GPU模型初始化...")
...
if __name__ == "__main__":
    logger.info("启动GPU-only RunPod handler...")
